...
def repeat_handler(bot, update):
    ...
    if worker:
        ...
        if hasattr(checks_workers, exec_worker):
            worker_call = getattr(checks_workers, exec_worker)
            return worker_call(bot, update, USER_DATA)

        elif hasattr(reports_workers, exec_worker):
            worker_call = getattr(reports_workers, exec_worker)
            return worker_call(bot, update, USER_DATA)
